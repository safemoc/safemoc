...
python 使用了分代回收机制:
将变量标记为三种状态:G0-G1-G2
对象刚刚创建的时候为G0 经过扫描存活下来的次数达到一定的量级,就移至G1扫描次数就会减少
G2 扫描次数会更少
'''
...
'''
python 是一种 解释性 强类型 动态语言
# 什么是强类型:
强类型: 数据类型不可被忽略,当一个变量被指定了某种数据类型 如果不经过强制转换,那么他就永远是这个类型了
弱类型: 数据类型可以被忽略,随着调用方式的不同,数据类型可以随意被切换 举例: php js

动态型|静态型
# 动态:
动态型语言是值运行的时候才进行数据类型的检查 变量在定义的时候根本不会指定数据类型 而是在赋值之后才会确定变量的数据类型
# 静态:
在定义的时候就要规定好 是什么类型  如果声明为整型 赋值为字符串就会直接报错
